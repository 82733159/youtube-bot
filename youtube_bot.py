...
def fetch_info(url: str) -> dict:
    ydl_opts = {**get_ydl_base_opts(), "skip_download": True}
...
async def cmd_cookiestatus(event):
    has_token  = _has_oauth2_token()
    has_cookie = COOKIES_FILE.exists()

    if has_token:
        await event.reply(
            "✅ **OAuth2 active — permanent auth!**\n"
            "Token cached by yt-dlp, auto-refreshes forever.\n\n"
            "_No maintenance needed._"
        )
    elif has_cookie:
        await event.reply(
            f"⚠️ **Using cookies.txt** (expires periodically)\n"
            f"`{COOKIES_FILE}`\n\n"
            "For permanent auth, send /login"
        )
    else:
        await event.reply(
            ...
            "Then send /login — done once, works forever."
        )
...
async def handle_message(event):
    ...
    buttons = []
    for i, opt in enumerate(video_list):
        size_str = f" ({human_size(opt['filesize'])})" if opt["filesize"] else ""
        buttons.append([Button.inline(f"{opt['label']}{size_str}", data=f"dl_v_{user_id}_{i}")])
    for i, opt in enumerate(audio_list):
        size_str = f" ({human_size(opt['filesize'])})" if opt["filesize"] else ""
        buttons.append([Button.inline(f"{opt['label']}{size_str}", data=f"dl_a_{user_id}_{i}")])
    ...
    if thumb_dl and thumb_dl.exists():
        await client.send_file(chat_id, file=str(thumb_dl), caption=caption, buttons=buttons)
    else:
        await client.send_message(chat_id, caption, buttons=buttons)


@client.on(events.CallbackQuery(pattern=rb"dl_(v|a)_(\d+)_(\d+)"))
async def handle_download(event):
    ...
    try:
        ...
        if kind == "v":
            ydl_opts = {
                **get_ydl_base_opts(),
                "format": f"{opt['format_id']}+bestaudio/best",
